...
const Header = () => {
  ...
  useEffect(() => {
    const loadSettings = async () => {
      try {
        ...
        cogoToast.error(`Failed to load application settings: ${error?.message || error}`);
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchSummaries = async () => {
      try {
        const resultAction = await dispatch(getStructureSummariesByWorkspaceId(workspaceId));
        ...
        cogoToast.error(
          `Error fetching structure summaries: ${error?.message || error || error?.data?.message}`,
        );
      }
    };
    ...
    <header className="bg-gray-100 flex justify-between items-center">
      <Navbar fluid rounded className="w-full p-4">
        <Navbar.Brand href="#" className="flex items-center gap-2">
          <FallbackImage src={logoUrl} alt={appName} className="h-8 w-8" />
          <span className="text-2xl font-bold text-custom-main uppercase">{appName}</span>
        </Navbar.Brand>
            ...
            <div className="absolute left-0 right-0 z-10 mt-1 bg-white border border-gray-300 rounded-md shadow-lg max-h-60 min-w-[40vw] w-full overflow-auto">
              {filteredSummaries.length > 0 ? (
                filteredSummaries.map((item) => (
                  ...
                  >
                    {item.name}
                    <span className="ml-1 text-gray-400 text-xs">({item.type})</span>
                  </div>
                ))
              ) : (
                <div className="px-4 py-2 text-sm text-gray-500">No matching structures found.</div>
              )}
            </div>
          )}
        </div>
