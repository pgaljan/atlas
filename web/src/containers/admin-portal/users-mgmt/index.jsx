...
const index = () => {
  ...
  const options = ['Personal', 'Analyst', 'Business', 'Educator', 'User', 'Admin'];
  ...
  const toggleUserStatus = async (id) => {
    ...
    try {
      await dispatch(updateUser({ userId: id, updateData: { status: newStatus } })).unwrap();
      cogoToast.success('User status updated successfully!');
      setTableData((prevData) =>
        prevData.map((user) => (user.id === id ? { ...user, status: newStatus } : user)),
      );
    } catch (error) {
  ...
  const handleUpdateUser = async (data) => {
    ...
    try {
      await dispatch(updateUser({ userId: editingUser.id, updateData })).unwrap();
  ...
  const toggleAdminStatus = async (id) => {
    ...
    try {
      ...
      setTableData((prevData) => prevData.map((u) => (u.id === id ? updatedUser : u)));
      fetchUsersData();
  ...
  const handleTierChange = async (userId, selectedPlanName) => {
    try {
      ...
      await dispatch(updateSubscriptionPlan({ userId, planId: selectedPlan.id })).unwrap();
  ...
  if (filteredUsers.length === 0 && !searchTerm === '') {
    return (
      <div className="flex h-screen flex-col items-center justify-center text-center p-6">
        ...
        <h2 className="text-2xl font-bold text-custom-text-grey mb-2">No users found</h2>
        <p className="text-lg text-custom-text-grey mb-4">
          There are no users to display. <br /> Please add a new user to get started.
        </p>
        {/* <button
            onClick={() => openModal()}
        ...
        <DeleteModal
          ...
          loading={deleting}
          title={userToDelete ? userToDelete.displayName || userToDelete.username : 'User'}
        />
      </div>
    );
  }
    ...
    <div className="p-2">
      <div className="p-10 rounded-[18px] bg-custom-background-white h-auto max-h-[90%] shadow-md">
        <div className="space-y-6">
          <h2 className="text-3xl font-semibold text-gray-800 mb-4 sm:mb-0">Users Management</h2>
        ...
        <GenericModal
          ...
          onSubmit={editingUser ? handleUpdateUser : handleSaveUser}
          type="user"
          title={editingUser ? `Update ${editingUser.displayName}` : 'Add User'}
          initialData={initialModalData}
        />
        <DeleteModal
          isOpen={isDeleteModalOpen}
            ...
            <thead className="border-b border-gray-100">
              <tr>
                {headers.map((header, index) => (
                  <th key={index} className="px-5 py-6 text-left border-b text-black-100">
                    {header}
                  </th>
                ))}
              </tr>
                  ...
                  <tr key={user.id} className="border-b border-gray-100">
                    <td className="px-5 py-4 sm:px-6">
                      <div className="flex items-center gap-3">
                        {user.profileUrl ? (
                          ...
                          <Avatar name={user.username} size="36" round={true} className="text-lg" />
                        )}

                        <div>
                          <p className="text-gray-600 font-medium">{user.displayName}</p>
                      ...
                      <label className="relative inline-flex items-center cursor-pointer">
                        ...
                        <div
                          className={`w-12 h-6 rounded-full transition-all ${
                            user.isAdmin === true ? 'bg-custom-main' : 'bg-gray-300'
                          }`}
                        ></div>
                    ...
                    <td className="px-4 py-3">
                      <label className="relative inline-flex items-center cursor-pointer">
                        ...
                        <div
                          className={`w-12 h-6 rounded-full transition-all ${
                            user.status === 'active' ? 'bg-custom-main' : 'bg-gray-300'
                          }`}
                        ></div>
                        <div
                    ...
                    <td className="px-4 py-3">
                      <input
                        ...
                        onBlur={(e) => handleInviteUpdate(user.id, e.target.value)}
                    ...
                    <td className="px-2 py-2 flex mt-2">
                      <Tooltip label="User info">
                        <button
                          ...
                          aria-label={`Show info for ${user.displayName || user.username}`}
                        >
        ...
        {isMetricsModalOpen && (
          <div className="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg shadow-lg p-6 w-[500px]">
              ...
              <p className="mb-4 text-gray-600">Select a date range to export user metrics data.</p>
              ...
              <div className="flex justify-end gap-2">
                ...
                <button
                  ...
                  className={`px-4 py-2 rounded text-white ${
                    startDate ? 'bg-green-600 hover:bg-green-700' : 'bg-gray-400 cursor-not-allowed'
                  }`}
                >
                  Export JSON
                </button>
