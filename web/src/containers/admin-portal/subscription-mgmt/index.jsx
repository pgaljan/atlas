...
const index = () => {
  ...
  const fetchPlansData = useCallback(async () => {
    ...
    try {
      ...
      cogoToast.error(error?.message || 'Failed to fetch plans. Please try again later.');
      setTableData([]);
    } finally {
      setLoading(false);
    }
  ...
  const toggleUserStatus = (id) => {
    ...
    dispatch(updatePlan({ id, updatePlanDto: { status: newStatus } }))
      .unwrap()
      .then((updatedPlan) => {
        setTableData((prevData) => prevData.map((p) => (p.id === id ? updatedPlan : p)));
  ...
  const handleEditPlan = async (updatedPlan) => {
    try {
      ...
      setTableData((prevData) => prevData.map((plan) => (plan.id === result.id ? result : plan)));
  ...
  const confirmDelete = () => {
    ...
    dispatch(deletePlan(selectedPlan.id))
      .unwrap()
      .then(() => {
        setTableData((prevData) => prevData.filter((item) => item.id !== selectedPlan.id));
    ...
    return (
      <div className="flex h-screen flex-col items-center justify-center text-center p-6">
        ...
        <p className="text-lg text-custom-text-grey mb-4">
          There are no subscription plans to display at the moment. <br /> Please add a new
          subscription plan.
        </p>
        <button
          ...
          <table className="p-10 w-full">
            <thead className="border-b border-gray-100">
              <tr>
                ...
                <th className="px-5 py-3 text-left text-black-100">Plan Name</th>
                <th className="px-5 py-3 text-left text-black-100">Description</th>
                <th className="px-4 py-3 text-left text-black-100">Price</th>
                <th className="px-4 py-3 text-left text-black-100">Status</th>
                <th className="px-2 py-3 text-left text-black-100">Actions</th>
              </tr>
            </thead>
                      ...
                      return (
                        <Draggable key={plan.id} draggableId={plan.id.toString()} index={index}>
                          {(provided) => (
                            ...
                            >
                              <td className="px-4 py-3 cursor-grab" {...provided.dragHandleProps}>
                                <TbDragDrop />
                              </td>
                              <td className="px-5 py-4 text-gray-500">{plan.name || '—'}</td>
                              <td className="px-5 py-4 text-gray-500">{plan.description || '—'}</td>
                              <td className="px-4 py-3 text-gray-500">$ {plan.price || '0.00'}</td>
