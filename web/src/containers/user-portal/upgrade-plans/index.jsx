...
import { fetchSubscription, updateSubscriptionPlan } from '../../../redux/slices/subscriptions';
        ...
        {sortedPlans?.map((plan, index) => (
          ...
          >
            ...
            <div className="flex flex-col mb-4">
              ...
              <p className="mt-1 text-sm sm:text-base text-custom-main">{plan?.description}</p>
            </div>

            <div className="mt-2 text-2xl sm:text-3xl md:text-4xl font-bold text-custom-main">
              {plan?.price && (
                <>
                  <span className="text-3xl font-bold">${plan?.price?.split('.')[0]}.0</span>
                  {plan?.price?.split('.')[1] && (
                    <sup className="text-xl font-bold">{plan?.price?.split('.')[1]}</sup>
                  )}
                </>
              )}
              {plan.price !== 'Free' && (
                <div className="mt-2 text-base font-normal text-custom-main">Per Month</div>
              )}
            </div>
            ...
            <div className="mt-6 flex-grow">
              <h4 className="text-base font-semibold text-custom-main">{plan?.featureHeading}</h4>
