...
import { assignNodeColors, assignWbsNumbers, treeToMarkmapData } from './markmapHelpers';
...
export const exportAsDoc = (
  ...
  const processNode = async (node) => {
    ...
    const elementTags = Array.isArray(node.tags)
      ? node.tags.map((tag) => `${tag.key}: ${tag.value}`).join(', ')
      : '';

    if (node.Record) {
      const { Record: record } = node;
      const editorType = record.editorType || '';
      const renderer = record.renderer;

      let recordContent = '';
      if (editorType === 'vscode' && renderer === 'markeddown') {
        recordContent = record.recordSvg || record.metadata?.content || '';
      }
        ...
        if (editorType === 'markeddown') {
          ...
          if (svgElement) {
            ...
            try {
              ...
              if (editorType === 'plantuml') {
                svgContent = `<img src="${imageDataURL}" style="max-width:40%; height:auto; display:block; margin:10px auto;" />`;
              } else {
  ...
  img { max-width: 100%; margin: 10px 0; }
        ...
        <body>
          ...
          ${svgContent}
          ${includeTags && elementTags ? `<p><strong>Tags:</strong> ${elementTags}</p>` : ''} 
        </body>
      </html>
    `;

      const fileName = `${wbsPrefix}${elementName.replace(/\s+/g, '_')}_${filenameTimestamp}.doc`;
...
<html>
  ...
  img { max-width: 100%; margin: 10px 0; }
</style>    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/markmap-toolbar@0.18.8/dist/style.css" />
  ...
  <body>
    ...
    zip
      ...
      .then((content) => {
        ...
        link.download = `${structureTitle.replace(/\s+/g, '_')}_${filenameTimestamp}.zip`;
        link.click();
      })
      .catch(() => {
...
export const exportAsPdf = async (
  ...
  const now = new Date();
  const filenameTimestamp = now.toISOString().replace(/[:.]/g, '-');
  const structureTitle = treeData && treeData.content ? treeData.content : 'Markmap Export';
  ...
  const processNode = async (node) => {
    ...
    if (node.Record) {
      ...
      const recordTags = record.tags?.map((tag) => `${tag.key}: ${tag.value}`).join(', ') || '';
          ...
          if (svgElement) {
            ...
            await new Promise((resolve) => {
              img.onload = () => {
                ...
                doc.addImage(dataUrl, 'PNG', 10, yOffset, targetWidth, targetHeight);
      ...
      if (editorType !== 'markeddown' && editorType !== 'plantuml' && editorType !== 'vscode') {
...
<html>
  ...
  `;
  zip.file(`${structureTitle.replace(/\s+/g, '_')}_${filenameTimestamp}_export.html`, htmlContent);
  ...
  zip
    ...
    .then((content) => {
      const link = document.createElement('a');
      link.href = URL.createObjectURL(content);
      link.download = `${structureTitle.replace(/\s+/g, '_')}_${filenameTimestamp}_pdf.zip`;
      link.click();
    })
    .catch(() => {
      cogoToast.error('Failed to generate ZIP file.');
  ...
  const processNode = async (node) => {
    ...
    if (node.Record) {
      ...
      if (editorType === 'quilleditor') {
        recordContent = record.metadata?.content || '<p>No content</p>';
      } else if (editorType === 'vscode') {
        if (renderer === 'markeddown') {
          recordContent = '';
        } else {
          const rawSrc = record.metadata?.content || '';
          recordContent = rawSrc
      ...
      if (record.recordSvg) {
        if (renderer === 'markeddown') {
          recordContent = record.recordSvg;
        } else {
          const tempContainer = document.createElement('div');
          Object.assign(tempContainer.style, {
            position: 'absolute',
          ...
          if (svgElement) {
            ...
            try {
              ...
              if (editorType === 'plantuml') {
                svgContent = `<img src="${imageDataURL}" style="height:150px; width:auto; display:block; margin:10px auto;" />`;
              } else {
                svgContent = `<img src="${imageDataURL}" style="max-width:100%; height:auto" />`;
              }
            } catch (err) {
...
${includeTags && elementTags ? `<p><strong>Tags:</strong> ${elementTags}</p>` : ''}
  ...
  link.download = `${structureTitle.replace(/\s+/g, '_')}_${filenameTimestamp}.doc`;
  link.click();
};

export const exportAsSinglePdf = async (treeData, showWbs, includeWbs, includeTags) => {
  if (!treeData || !treeData.children || treeData.children.length === 0) {
    cogoToast.warn('No elements found to export.');
    return;
  }
  ...
  const processNode = async (node) => {
    ...
    if (node.Record) {
      const record = node.Record;
      const editorType = record.metadata?.editorType;
      const isRawContentOnly = ['markeddown', 'vscode', 'plantuml'].includes(editorType);
      const recordContent = isRawContentOnly ? null : record.metadata?.content || 'No content';
      const recordTags = record.tags?.map((tag) => `${tag.key}: ${tag.value}`).join(', ') || '';
...
export const exportAsHtml = (treeData, includeWbs, colorStrategy, isMarkmap = false) => {
  if (!treeData) {
...
<html>
  ...
  const link = document.createElement('a');
  link.href = URL.createObjectURL(blob);
  link.download = `${structureTitle.replace(/\s+/g, '_')}_${filenameTimestamp}_export.html`;
  link.click();
};
